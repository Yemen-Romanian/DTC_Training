...
# ----------------------------------------------------------------------------------------------------------------------
#
# A modified version of MLFLower from https://github.com/dryabokon/tools/blob/master/tools_MLflower.py
class MLFlower(object):
    ...
    def scp_file_to_remote(self,local_file_path, remote_folder):
        ...
        if self.ppk_key_path is not None:
            command = f'scp -o StrictHostKeyChecking=no -P {int(22)} -i {self.ppk_key_path} {local_file_path} {self.username_ssh}@{self.host_ssh}:{remote_folder}'
            os.system(command)
...
# ---------------------------------------------------------------------------------------------------------------------
    ...
    def get_uris(self):
        ...
        return artifact_uri
    # ---------------------------------------------------------------------------------------------------------------------
    def log_datasets(self, datasets):
        """Log dataset lineage for the current active run.

        `datasets` is a list of descriptors, one per split/source, each a dict with:
            name    (str)       -- display name shown in the Datasets panel, e.g. "train_synthetic"
            source  (str)       -- local folder path the data was read from
            context (str)       -- split tag: "training" | "validation" | "testing"
            samples (list[str]) -- names of the individual samples (e.g. video labels) in this split

        Each descriptor becomes a MetaDataset (source + content-based digest) logged via
        mlflow.log_input. The full per-split sample listing is also written as a browsable
        JSON artifact so you can open a run and see exactly which sequences were used.
        """
        samples_index = {}
        for ds in datasets:
            samples = sorted(ds.get('samples', []))
            # Digest reflects the actual set of samples, so runs with different
            # sequences get different digests even when the folder path is unchanged.
            digest = hashlib.sha256(json.dumps(samples).encode('utf-8')).hexdigest()[:8]
            source = LocalArtifactDatasetSource(uri=ds['source'])
            meta_dataset = MetaDataset(source=source, name=ds['name'], digest=digest)
            mlflow.log_input(meta_dataset, context=ds.get('context'))
            samples_index[ds['name']] = samples

        mlflow.log_dict(samples_index, 'dataset_samples.json')
        return
    # ---------------------------------------------------------------------------------------------------------------------
    def save_experiment(self, experiment_name, params={}, metrics={}, artifacts=[], model=None, registered_model_name=None, datasets=None):
        ...
        with mlflow.start_run(experiment_id=self.get_experiment_id(experiment_name, create=True),run_name=pd.Timestamp.now().strftime('%Y-%b-%d %H:%M:%S')) as run:
            ...
            if model is not None:
                mlflow.pytorch.log_model(model, artifact_path="model", registered_model_name=registered_model_name)

            if datasets:
                self.log_datasets(datasets)
